...
bool isAccessible(int current_x, int current_y, int target_x, int target_y)
{
  ...
  return false; // if the target is not adjacent to current
}
...
bool isIncrementConsistent(int current_x, int current_y, int (&floodArray)[3][4])
{
...
void makeCellConsistent(int current_x, int current_y, int (&floodArray)[3][4])
{
...
void floodFillUsingQueue(int start_x, int start_y, int previous_x, int previous_y, int (&floodArray)[3][4])
{
  int north_X, north_Y, east_X, east_Y, south_X, south_Y, west_X, west_Y;
...
char whereToMove(int current_x, int current_y, int previous_x, int previous_y, int orient, int (&floodArray)[3][4])
{
...
int run_maze(int (&floodArray)[3][4])
{
  int Orient = 0;
